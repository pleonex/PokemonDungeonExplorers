...
namespace DungeonKey.Rounds
{
    ...
    public static class Crc32
    {
        ...
        /// <summary>
        /// Calculate the CRC32 from the data.
        /// </summary>
        /// <param name="data">Data for the CRC.</param>
        /// <param name="startIdx">Start index of the data.</param>
        /// <param name="length">Number of bytes to read.</param>
        public static uint Calculate(byte[] data, int startIdx, int length)
        {
            uint crc = 0xFFFFFFFF;
            for (int i = 0; i < length; i++) {
                uint tableIdx = (crc ^ data[startIdx + i]) & 0xFF;
                crc = Table[tableIdx] ^ (crc >> 8);
            }
